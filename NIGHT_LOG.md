...
**Реализация:**
- `cmdSpreadAnalysis() void` — получает все погодные рынки, анализирует спреды по 5 диапазонам
- Диапазоны: ≤0.01 (Tight ✅), 0.01-0.03 (Good), 0.03-0.05 (Fair), 0.05-0.10 (Wide ⚠️), >0.10 (Very Wide ❌)
- Главная таблица: Spread Range | Count | % of Total | Avg Volume | Status emoji
- Вторая секция: Per-City Spread Summary с минимум/средний/максимум по каждому городу, сортировка по среднему спреду (лучшие первыми)
- Эмодзи по качеству: 🟢 avg≤0.02 | 🟡 avg≤0.05 | 🔴 avg>0.05
- Итоговая статистика: всего рынков и % с узким спредом ≤0.01
- Добавлен case в switch statement main() и в printUsage()

**Результат:** `go build ./...` ✅
**Строк кода:** 155 в cmdSpreadAnalysis()
**Файлы изменены:** `cmd/dashboard/main.go` (1 файл)

---

## 2026-05-28 08:48 UTC — TASK-194

**Задача:** Реализовать `/trend` Telegram команду для показа 7-дневного тренда выбранного города.

**Реализация:**
- `handleTrend(city string, bcfg BotConfig) string` — загружает историю ставок, группирует по дням (последние 7 дней)
- Если город не указан — выводит список доступных городов с инструкциями
- Таблица показывает для каждого дня: количество ставок, win rate %, avg edge, P&L
- Итоговая строка с суммарной статистикой (Total Bets, P&L)
- Тренд-индикатор: ↑ (if best day > +100), ↓ (if worst day < -100), → (стабильно)
- Обновлена документация команд
- Добавлен case `/trend` в switch statement поллера с обработкой опционального аргумента города

**Результат:** `go build ./...` ✅
**Строк кода:** 122 в handleTrend()
**Файлы изменены:** `internal/notifier/telegram_commands.go` (1 файл)

---

## 2026-05-28 08:42 UTC — TASK-192

**Задача:** Реализовать `/compare` Telegram команду для сравнения статистики сегодня vs вчера.

**Реализация:**
- `handleCompare(bcfg BotConfig) string` — загружает историю ставок, вычисляет метрики для сегодня и вчера
- Сравниваемые метрики: Total Bets, Resolved, Win Rate %, Avg Edge, PnL USDC, ROI %
- Таблица показывает дельта (Δ%) между днями с цветовой индикацией тренда (↑ 🟢 улучш / → 🟡 стабильно / ↓ 🔴 ухудш)
- Обновлена документация команд (строки 4-5)
- Добавлен case `/compare` в switch statement поллера

**Результат:** `go build ./...` ✅
**Строк кода:** 117 в handleCompare()
**Файлы изменены:** `internal/notifier/telegram_commands.go` (1 файл)
...
## 2026-05-28 05:42 UTC — TASK-154 + TASK-155

### TASK-154: Bet export CSV via `/export` Telegram command

Добавил `/export [days]` команду, которая отправляет CSV-файл с историей resolved ставок за последние N дней (default: 30).

### Новые файлы
- нет

### Изменённые файлы
- `internal/notifier/telegram_commands.go` — добавлен `/export` case в switch; новые функции `handleExport` (~70 строк) и `sendDocument` (~45 строк)

**Детали:**
- CSV колонки: `timestamp,condition_id,city,signal,side,size_usdc,market_price,our_probability,outcome,resolved_at,pnl_usdc`
- Фильтр: только resolved bets за указанный период
- ≥ 50 строк → gzip-сжатие, расширение `.csv.gz`
- Использует Telegram sendDocument API через multipart/form-data

---

### TASK-155: Forecast freshness guard

Вынес логику проверки свежести прогноза в отдельный файл `freshness.go` с именованными функциями. Добавил счётчик `stale_forecasts_skipped_total` в Prometheus /metrics.

### Новые файлы
- `internal/collectors/freshness.go` — `ForecastAge()`, `IsForecastStale()` (~30 строк)
- `internal/collectors/freshness_test.go` — 8 unit-тестов (~75 строк)

### Изменённые файлы
- `internal/metrics/metrics.go` — новый `staleForecastsSkipped` atomic counter; `IncStaleForecastSkipped()`; метрика в `/metrics`
- `cmd/bot/main.go` — заменён inline код TASK-029 на `collectors.IsForecastStale()` + `metrics.IncStaleForecastSkipped()`; убрана функция `maxForecastAge` (теперь не нужна)

**Тесты:** 8/8 зелёных — fresh, boundary, stale, disabled(0), nil, zeroFetchedAt, approxAge, zeroAge.

`go build ./...` ✅  `go test ./internal/collectors/... -run "TestIsForecastStale|TestForecastAge"` ✅

**Итого:** ~160 строк, 5 файлов (2 новых + 3 изменённых).

---

## 2026-05-28 05:47 UTC — TASK-156 + TASK-157

### TASK-156: Per-signal adaptive Kelly multiplier

Сигналы с хорошим историческим Brier score теперь получают автоматически более высокий Kelly cap. Плохо откалиброванные сигналы — снижаются.

### Новые файлы
- `internal/calibration/signal_kelly.go` — `SignalKellyMultipliers()`, `LookupSignalKelly()`, `SignalKellyInfo` struct + Brier→multiplier таблица (~75 строк)
- `internal/calibration/signal_kelly_test.go` — 7 unit-тестов: пустая история, недостаточно данных, отличная/плохая калибровка, несколько сигналов, пропуск по умолчанию, String() (~100 строк)

### Изменённые файлы
- `cmd/bot/main.go` — добавлено `signalKellyMults := calibration.SignalKellyMultipliers(history)` раз в цикл; применяется после Platt + timing multiplier; логирует debug + добавляет в Decision.Reason (~45 строк)

**Таблица мультипликаторов:**
- Brier < 0.10 → 1.50x (отличная калибровка)
- Brier < 0.14 → 1.20x (хорошая)
- Brier < 0.18 → 1.00x (базовый)
- Brier < 0.22 → 0.75x (слабый)
- Brier ≥ 0.22 → 0.50x (плохой — резко снижаем)

`go build ./...` ✅  `go test ./internal/calibration/... -run TestSignalKelly` ✅ (7/7)

---

### TASK-157: `/healthcheck` Telegram команда

Добавлена команда `/healthcheck` для получения комплексного health-отчёта прямо в Telegram.

### Изменённые файлы
- `internal/notifier/telegram_commands.go` — добавлен `case "/healthcheck"` в switch; функция `handleHealthcheck(bcfg BotConfig) string` (~110 строк); добавлены import `sort` и `risk`; добавлено поле `StartTime time.Time` в `BotConfig`; обновлён docstring
- `cmd/bot/main.go` — заполнение `StartTime: sess.startTime` в вызове `StartCommandPoller`

**Отчёт включает:**
- Uptime и pause-статус
- Статус каждого источника данных: последний успешный fetch, uptime%, consec_fails
- Brier score, rolling win rate, drift alert, streak alert
- Daily risk state: bets today, daily P&L, open positions, bankroll
- Per-signal Kelly мультипликаторы (только отличающиеся от 1.0x)
- Timestamp UTC

`go build ./...` ✅  `go test ./internal/notifier/...` ✅

**Итого TASK-156+157:** ~330 строк, 5 файлов (2 новых + 3 изменённых).

---

## 2026-05-28 03:57 UTC — TASK-158 + TASK-159 + TASK-160

Все предыдущие задачи (157 штук) были выполнены. Добавил 3 новых задачи и реализовал их.

### TASK-158: Precipitation climatological z-score

Зеркало TASK-064 (ClimateAnomalyScore для температуры), но для осадков. Когда прогноз предсказывает аномально высокие осадки (z > 2σ), confidence повышается до 0.72 floor — heavy rain события хорошо разрешаются NWP моделями.

**Новые файлы/изменения:**
- `internal/weather/seasonal.go` — добавлен `precipHistShape` struct и `PrecipitationZScore(city, precipMM, dataRoot) float64` (~65 строк)
- `internal/collectors/aggregator.go` — добавлено поле `PrecipZScore float64` в `FusedForecast`; в `AggregateForDay`: вычисление и confidence boost при z > 2.0 (~18 строк)
- `internal/weather/seasonal_test.go` — 5 unit-тестов: missing file, too few records, arid city (sigma < 0.5), heavy rain anomaly, dry anomaly (~65 строк)

### TASK-159: `dashboard markets` subcommand

Новый subcommand `go run ./cmd/dashboard markets` — таблица всех активных Polymarket погодных рынков с ценами, спредом, статусом (Active/Thin/Stale) и временем до истечения. Сортировка: Active → Thin → Stale.

**Изменения:**
- `cmd/dashboard/main.go` — добавлены: `cmdMarkets()`, `marketStatusRank()`, `marketStatusLabel()`, `formatDuration()`; case в switch; строка в printUsage (~100 строк)

### TASK-160: `/source-weights` Telegram команда

Показывает текущие динамические веса источников данных (dynamic vs static), Brier score и кол-во наблюдений для каждого источника. Если динамические данные недоступны — сообщает об этом.

**Изменения:**
- `internal/notifier/telegram_commands.go` — добавлены: `handleSourceWeights()`, `repeatDash()`; case в switch; строка в docstring; `/source-weights` в поллере (~70 строк)

`go build ./...` ✅  `go test ./...` ✅ (все тесты прошли)

**Итого:** ~320 строк, 5 файлов изменено

---

## 2026-05-28 06:12 UTC — TASK-161 + TASK-162 + TASK-163

Все предыдущие задачи (160 штук) были выполнены. Добавил 3 новые задачи и реализовал их все.

### TASK-161: `CityPnL` + `dashboard pnl-city`

Новая утилитная функция для P&L breakdown по городам (и по сигналам). Используется в dashboard и Telegram.

**Новые файлы:**
- `internal/calibration/pnl_city.go` — `CityPnL()`, `SignalPnL()`, `CityPnLStats` struct с методами `WinRate()` и `ROI()` (~100 строк)
- `internal/calibration/pnl_city_test.go` — 6 unit-тестов: empty, skip unresolved, skip no city, win/loss sort, winrate/ROI, signal basic (~100 строк)

**Изменённые файлы:**
- `cmd/dashboard/main.go` — `cmdPnLCity(dataRoot)` функция (~65 строк); case в switch; строка в printUsage

### TASK-162: `MinBetUSDC` конфиг

Унифицировал 4 хардкода `< 0.5` в strategy.go в один configurable `MinBetUSDC` package var.

**Изменённые файлы:**
- `config/config.go` — поле `MinBetUSDC float64`, default 0.50, ENV: `MIN_BET_USDC`
- `internal/strategy/strategy.go` — `var MinBetUSDC = 0.50`; заменены 4 хардкода
- `cmd/bot/main.go` — `strategy.MinBetUSDC = cfg.MinBetUSDC` при старте

### TASK-163: Telegram `/pnl-city`

**Изменённые файлы:**
- `internal/notifier/telegram_commands.go` — `handlePnLCity()` (~55 строк); case в switch; обновлён docstring

`go build ./...` ✅  `go test ./...` ✅ (все тесты прошли)

**Итого:** ~320 строк, 7 файлов (2 новых + 5 изменённых).

---
## 2026-05-28 06:35 UTC — Iteration: TASK-164 + TASK-165 + TASK-166

**Все предыдущие задачи были выполнены. Добавлены и реализованы 3 новые задачи.**

### Что сделано

**TASK-164: `dashboard week` — 7-дневная таблица P&L**
- `DailyPnLTable(records, nDays) []DailyStats` в `internal/calibration/pnlchart.go` (+57 строк)
- `DailyStats{Date, Bets, Wins, PnLUSDC, CumulativePnL}` + `WinPct()` метод
- `cmdWeek(dataRoot, nDays)` в `cmd/dashboard/main.go` — таблица с цветовой кодировкой (+63 строки)
- Case `"week"` в switch + строка в `printUsage()`

**TASK-165: Proactive opportunity alert**
- `OpportunityAlertThreshold float64` в `config/config.go` (yaml + ENV `OPPORTUNITY_ALERT_THRESHOLD`)
- `SendOpportunityAlert(d *strategy.Decision)` в `internal/notifier/telegram.go` (+37 строк)
- `notifiedOpportunities sync.Map` в `cmd/bot/main.go` — дедупликация по conditionID
- Интеграция в главный цикл после signal kelly применения
- Добавлено в `.env.example`

**TASK-166: `internal/calibration/weekly.go` — недельная агрегация + тесты**
- `WeeklyStats`, `WeeklyBreakdown(records, nWeeks)`, `BestWeek()`, `WorstWeek()` (+98 строк)
- `internal/calibration/weekly_test.go` — 5 unit-тестов, все проходят (+82 строки)

### Файлы
- `internal/calibration/pnlchart.go` (+57 строк)
- `internal/calibration/weekly.go` (новый, 98 строк)
- `internal/calibration/weekly_test.go` (новый, 82 строки)
- `internal/notifier/telegram.go` (+37 строк)
- `cmd/dashboard/main.go` (+67 строк, +import strconv)
- `cmd/bot/main.go` (+17 строк, +import sync)
- `config/config.go` (+7 строк)
- `.env.example` (+5 строк)
- `TASKS.md` (добавлены TASK-164..166, отмечены выполненными)

### Итого: ~370 строк нового кода. `go build ./...` ✅ `go test ./...` ✅

---
## 2026-05-28 06:42 UTC — Iteration: TASK-167 + TASK-168 + TASK-169

**Все предыдущие задачи были выполнены. Добавлены и реализованы 3 новые задачи.**

### Что сделано

**TASK-167: `/explain <conditionID>` Telegram команда**
- `handleExplainMarket(bcfg, conditionID)` в `internal/notifier/telegram_commands.go` (~65 строк)
- Ищет рынок среди GetWeatherMarkets() по conditionID (регистронезависимо)
- Запускает `strategy.ExplainEvaluate` и форматирует полный аудит: confidence/consensus/sources, RawP→SeasonP→FinalP, YES/NO edge, Kelly, EnsScale, итог BET/SKIP
- Добавлен case `/explain` в поллере с парсингом аргумента

**TASK-168: `dashboard signals-trend` — тренд win rate по сигналам (7/14/30 дней)**
- `SignalBreakdownForPeriod(records, days)` в `internal/calibration/calibration.go` (+14 строк) — фильтрует resolved ставки за последние N дней и вызывает SignalBreakdown
- `cmdSignalsTrend(dataRoot)` в `cmd/dashboard/main.go` (~100 строк) — таблица: Signal | 7d WR | 14d WR | 30d WR | 30d Brier | 30d N | Trend
- Цветовая кодировка: зелёный WR ≥ 55%, красный < 45%; стрелка ↑/↓/→ по разнице 7d vs 30d

**TASK-169: `/winrate` Telegram команда**
- `handleWinRate(bcfg)` в `internal/notifier/telegram_commands.go` (~65 строк)
- Группирует последние 20 resolved ставок per signal, вычисляет WinRate и P&L
- Таблица `<pre>`: Signal | N | Win% | PnL, итог TOTAL
- Добавлен case `/winrate` в поллере

### Файлы
- `internal/notifier/telegram_commands.go` (+135 строк, 2 новые функции + docstring + cases)
- `internal/calibration/calibration.go` (+14 строк: SignalBreakdownForPeriod)
- `cmd/dashboard/main.go` (+103 строки: cmdSignalsTrend + case + printUsage)
- `TASKS.md` (добавлены TASK-167..169, отмечены выполненными)

`go build ./...` ✅

### Итого: ~252 строки нового кода, 4 файла.

---
## 2026-05-28 07:00 UTC — Iteration: TASK-170 + TASK-171

**Все предыдущие задачи выполнены. Добавлены новые задачи TASK-170..173. Реализованы TASK-170 + TASK-171.**

### Что сделано

**TASK-170: Market discovery caching**
- `internal/markets/cache.go` (новый, 80 строк) — `loadMarketCache()` / `saveMarketCache()` с TTL 1h, disk path `data/markets_cache.json`
- `internal/markets/markets.go` (+6 строк) — cache hit/miss в `GetWeatherMarkets()`, сохранение после успешного фетча
- `cmd/bot/main.go` (+3 строки) — `markets.SetCacheDataRoot(cfg.DataRoot)` при старте
- Ускоряет рестарты: пропускает 10-30 сек Polymarket API пагинации при горячем кэше

**TASK-171: Losing streak Kelly reducer**
- `internal/calibration/streak.go` (новый, 70 строк) — `StreakResult`, `CurrentStreak()`, `StreakKellyFactor()`
- `internal/calibration/streak_test.go` (новый, 75 строк) — 6 unit-тестов (пустой, одиночный, 2 проигрыша, 3 проигрыша, сброс, Kelly факторы)
- `cmd/bot/main.go` (+14 строк) — применяет `streakFactor * KellyFraction` аналогично drawdownMult
- Защита: 2 подряд проигрыша → Kelly×0.85, 3+ → Kelly×0.70, при первой победе сброс

### Файлы
- `internal/markets/cache.go` (новый, 80 строк)
- `internal/markets/markets.go` (+6 строк)
- `internal/calibration/streak.go` (новый, 70 строк)
- `internal/calibration/streak_test.go` (новый, 75 строк)
- `cmd/bot/main.go` (+17 строк)

`go build ./...` ✅ `go test ./...` ✅ (все пакеты зелёные)

### Итого: ~248 строк нового кода, 5 файлов.

---
## 2026-05-28 07:02 UTC — Iteration: TASK-172 + TASK-173

### Что сделано

**TASK-172: `/config` Telegram команда**
- `internal/notifier/telegram_commands.go` — расширен `BotConfig` struct (DryRun, KellyFraction, MaxKellyFraction, LoopSec, ProtocolFeeRate); добавлен `handleConfig(bcfg BotConfig) string` — форматирует `<pre>` таблицу: mode/min_edge/max_bet/bankroll/kelly/max_kelly/protocol_fee/loop_interval/data_root; добавлен `case "/config"` в поллер; обновлен docstring
- `cmd/bot/main.go` — передаёт 5 новых полей в `notifier.BotConfig` при старте поллера

**TASK-173: `dashboard positions` — обновлена таблица открытых позиций**
- `internal/calibration/unrealized.go` — добавлена `FetchMarketEndDate(conditionID string) (time.Time, error)` через `queryGammaMarket`
- `cmd/dashboard/main.go` — переписан `cmdPositions`: новые колонки Time | City/Signal | Side | Size | Price | Hours Left; сортировка по времени экспирации (ближайшие сначала); итог "N open, total exposure USDC"; entry→current price отображается как "0.420→0.510"

### Файлы
- `internal/notifier/telegram_commands.go` (+50 строк)
- `cmd/bot/main.go` (+6 строк)
- `internal/calibration/unrealized.go` (+16 строк)
- `cmd/dashboard/main.go` (~80 строк изменено)

`go build ./...` ✅ `go test ./...` ✅

### Итого: ~152 строки, 4 файла.

---

## 2026-05-28 07:12 UTC — TASK-174: Weather Model Bias Tracker

**Что сделано:** Реализован систематический корректор смещения вероятностей per-(city, signal).

После каждого resolved бета резолвер записывает (ourP, outcome) в rolling window (30 записей) в `data/bias/{city}_{signal}.json`. При накоплении ≥5 записей функция `CorrectProbability` вычитает mean bias из ourP (clamp 0.02–0.98). Correction применяется в bot loop после Platt калибровки. Если скорректированный edge < min_edge → ставка пропускается.

**Файлы:**
- `internal/calibration/bias.go` (новый, ~165 строк) — BiasRecord, RecordBiasOutcome, ComputeBias, CorrectProbability, LoadBiasSummary, splitCitySignal
- `internal/calibration/bias_test.go` (новый, ~70 строк) — 7 unit-тестов: NoData, RecordAndCompute, CorrectNoData, ClampLower, SummaryEmpty, RollingCap, SplitCitySignal
- `internal/calibration/resolver.go` (обновлён) — вызов RecordBiasOutcome после UpdateOutcome
- `cmd/bot/main.go` (обновлён, ~25 строк) — bias correction после applyPlattCalibration
- `cmd/dashboard/main.go` (обновлён, ~55 строк) — subcommand "bias": таблица City|Signal|Bias|N|Status|Interpretation

**Результат:** `go build ./...` ✅ | `go test ./...` ✅ (все 9 пакетов OK)

**Строк кода:** ~315 (новые файлы + изменения)

---

## 2026-05-28 07:35 UTC — TASK-176, TASK-177, TASK-178

**Задачи:** Pre-trade slippage guard, Per-source timeout config, Brier score trend alert

### TASK-176: Pre-trade slippage guard
- `internal/markets/slippage.go` (новый, ~115 строк) — `EstimateSlippage()`, `FetchBookForToken()`, `CheckSlippage()`, `SlippageResult`
  - Симулирует исполнение ордера по CLOB стакану (ask-side для YES buy, bid-side для NO buy)
  - slippage > 0.07 → Skip=true (пропустить ставку); slippage > 0.03 → размер ×0.5
- `cmd/bot/main.go` (обновлён, ~20 строк) — pre-trade проверка перед каждой реальной ставкой

### TASK-177: Per-source timeout config
- `config/config.go` (обновлён, ~12 строк) — добавлен `SourceTimeouts map[string]int`; defaults: openmeteo=8, nasa=10, noaa=8, goes=15, hrrr=8, ensemble=10, ecmwf=12, gfs=10
- `config/config.yaml` (обновлён, ~14 строк) — задокументированы per-source таймауты
- `internal/collectors/aggregator.go` (обновлён, ~50 строк) — `SetSourceTimeouts()`, `sourceTimeout()`, `sourceTimeoutsMap`; каждая goroutine в `collectSources()` теперь создаёт child context с source-specific timeout
- `cmd/bot/main.go` (обновлён, ~1 строка) — `collectors.SetSourceTimeouts(cfg.SourceTimeouts)` при старте

### TASK-178: Brier score trend alert
- `internal/calibration/brier_trend.go` (новый, ~140 строк) — `weeklyBrierPoints()`, `linReg()`, `BrierTrend()`, `BrierTrendAlert()`, `BrierTrendLine()`
  - OLS линейная регрессия по недельным Brier score; slope > 0.015/нед И R² > 0.7 → alert
  - Минимум 3 qualifying недели с ≥5 ставок каждая
- `internal/calibration/brier_trend_test.go` (новый, ~115 строк) — 4 unit-теста
- `cmd/bot/main.go` (обновлён, ~8 строк) — вызов `BrierTrendAlert` при старте; Telegram уведомление + лог

**Результат:** `go build ./...` ✅ | `go test ./internal/calibration/ -run TestBrier` ✅ (все 4 новых теста OK)

**Строк кода:** ~475 (новые файлы + изменения)

---

## 2026-05-28 07:42 UTC — TASK-179, TASK-180, TASK-181

**Задачи:** /markets Telegram command, Hour-of-day win rate, Isotonic regression calibration

### TASK-179: `/markets` Telegram команда
- `internal/notifier/telegram_commands.go` (обновлён, ~65 строк) — `handleMarkets(bcfg BotConfig) string`
  - Вызывает `GetWeatherMarkets()` + `EnrichWithLiquidity()`, фильтрует по city+signal+non-stale
  - Сортировка по spread ASC (лучшие возможности первыми), top-5
  - Таблица: City | Signal | YES | NO | Spr | Expiry (hours/days)
  - Добавлен `/markets` в docstring и switch поллера

### TASK-180: Hour-of-day win rate
- `internal/calibration/pnlchart.go` (обновлён, ~40 строк) — `HourlyStats` struct + `HourlyWinRate(records)` функция
  - Группирует resolved ставки по UTC часу Timestamp → [24]HourlyStats
  - WinPct() метод на HourlyStats
- `cmd/dashboard/main.go` (обновлён, ~80 строк) — `cmdHourlyWinRate(dataRoot)`
  - go-pretty таблица: UTC Hour | Bets | Wins | Win% | P&L | Bar
  - ASCII бар (█░) пропорционально win rate, зелёный/красный для best/worst часа
  - Итог: best/worst час, total resolved bets
  - Добавлен в switch и printUsage

### TASK-181: Isotonic regression calibration
- `internal/calibration/isotonic.go` (новый, ~185 строк) — Pool Adjacent Violators algorithm
  - `IsotonicCalibrator{Xs, Ys, N}` struct с JSON persistence
  - `FitIsotonic(records)` — PAV O(n log n), weighted merging blocks
  - `Predict(rawP)` — кусочно-линейная интерполяция, clamp [0.02, 0.98]
  - `LoadIsotonic`, `SaveIsotonic`, `UpdateAndSaveIsotonic`
- `internal/calibration/isotonic_test.go` (новый, ~110 строк) — 5 unit-тестов: empty, monotone, violation, interpolation, clamping
- `config/config.go` (обновлён, ~4 строки) — `UseIsotonic bool` поле (yaml: `use_isotonic`)
- `cmd/bot/main.go` (обновлён, ~45 строк) — `applyIsotonicCalibration()`, выбор isotonic/Platt по cfg.UseIsotonic, UpdateAndSaveIsotonic в resolver callback

**Результат:** `go build ./...` ✅ | `go test ./internal/calibration/ -run TestIsotonic` ✅ (5/5 OK)

**Строк кода:** ~535 (новые файлы + изменения)

---

## 2026-05-28 07:52 UTC — TASK-182, TASK-183, TASK-184

**Задачи:** /top-edge Telegram command, Kelly fraction auto-tuner, Forecast stability tracker

### TASK-182: `/top-edge` Telegram команда
- `internal/notifier/telegram_commands.go` (обновлён, ~75 строк) — `handleTopEdge(bcfg BotConfig) string`
  - Загружает активные рынки через `markets.GetWeatherMarkets()`
  - Для каждого рынка: берёт прогноз из disk cache (до 3h), вычисляет `ourP` для нужного сигнала (heat/cold/rain/sunny/wind/snow/fog/humid/dry)
  - Вычисляет edge для YES и NO сторон, отбирает лучшую с положительным edge
  - Score = bestEdge × confidence; top-5 по score DESC
  - Колонки: City | Signal | Side | Edge | Conf | Score | Price
  - Добавлен `math` в imports, `/top-edge` в docstring и switch поллера

### TASK-183: Kelly fraction auto-tuner
- `internal/calibration/kelly_opt.go` (новый, ~110 строк) — `OptimalKelly(records, start, end, steps)`
  - Grid search по Kelly fraction 0.05–1.0 (20 шагов)
  - Для каждого k: симулирует reinvesting k × full-Kelly на каждой resolved ставке
  - Возвращает k с максимальным final bankroll (=geometric growth rate)
  - Hard cap 20% per bet в симуляции; graceful ruin detection (bankroll→0)
  - Минимум 10 resolved бетов, иначе ошибка
- `cmd/dashboard/main.go` (обновлён, ~45 строк) — `cmdKellyOpt(dataRoot)`
  - Таблица: K | Sim P&L | Log Growth | Quality (← optimal подсвечен зелёным)
  - Добавлен `math` в imports, `kelly-opt` case в switch и printUsage

### TASK-184: Forecast stability tracker
- `internal/collectors/stability.go` (новый, ~145 строк) — `StabilityTracker`
  - Rolling window 10 наблюдений per conditionID (sync.RWMutex)
  - `Track(conditionID, city, signal, ourP)` — добавляет запись
  - `Stability(conditionID) float64` — population stddev последних N оценок
  - `IsUnstable(conditionID) bool` — stddev > 0.15
  - `Snapshot() []StabilitySnapshot` — срез всех tracked markets для dashboard
  - `GlobalStability` — package-level singleton
- `internal/strategy/strategy.go` (обновлён, ~10 строк) — интеграция в `EvaluateFused()`
  - После вычисления ourP: `GlobalStability.Track(..., ourP)`
  - Если `IsUnstable` → `ff.Confidence *= 0.80` + slog warning с stddev
- `cmd/dashboard/main.go` (обновлён, ~55 строк) — `cmdStability(dataRoot)`
  - Читает сегодняшний prediction log (JSONL) — группирует OurP по (city, signal)
  - Таблица: City | Signal | N | Mean P | StdDev | Last P | Status (stable/⚠️unstable)
  - Сортировка по stddev DESC (самые нестабильные первыми)
  - `stability` case в switch и printUsage

**Результат:** `go build ./...` ✅

**Строк кода:** ~440 (новые файлы + изменения)

---

### 2026-05-28 08:12 UTC — TASK-186 + TASK-187 (iteration)

**Задачи:** все 185 предыдущих задач выполнены. Добавлены 3 новые задачи (186–188), реализованы первые две.

**TASK-186: Size-weighted Brier score**
- `internal/calibration/calibration.go` (~40 строк) — `WeightedBrierScore(records []BetRecord) (score, count, err)`
  - Вес каждого бета = SizeUSDC / mean(SizeUSDC) среди resolved
  - Показывает капитал-взвешенную точность (крупные ставки важнее мелких)
  - Логируется при старте бота рядом с обычным Brier
- `cmd/dashboard/main.go` (~14 строк) — в `cmdPnL()` добавлены строки "Weighted Brier" + "EV capture"

**TASK-187: EV capture ratio**
- `internal/calibration/ev.go` (новый, ~80 строк) — `EVResult{Count, ExpectedEV, RealizedPnL, CaptureRatio}`
  - `RollingEV(records, n int)` — последние n resolved ставок (0=все)
  - `RollingEVBySignal(records, n int)` — breakdown по сигналу
  - EV capture = realizedPnL / expectedEV (1.0 = захватываем весь edge, <0.7 = проблема)
- `internal/calibration/ev_test.go` (новый, ~100 строк) — 7 unit-тестов (все pass)
- `internal/notifier/telegram_commands.go` (~55 строк) — `/ev` команда с эмодзи ✅⚠️🚨
- `cmd/dashboard/main.go` (~80 строк) — `ev-track` subcommand с per-signal таблицей

**Результат:** `go build ./...` ✅ | `go test ./...` ✅ (все тесты pass)
**Строк кода:** ~370 (новые + изменения)
**Файлы:** calibration.go, ev.go (новый), ev_test.go (новый), telegram_commands.go, dashboard/main.go

---

## 2026-05-28 09:17 UTC — TASK-199 + TASK-200

### TASK-199: Per-cycle stats CSV
**Что сделано:**
- `internal/calibration/cycle_stats.go` (новый, ~110 строк): `CycleStat` struct, `AppendCycleStat()`, `LoadCycleStats()` — CSV с header guard, создаёт `data/cycles.csv`
- `cmd/bot/main.go`: добавлены поля `confSum/confCount` в `cycleResult`; замыкание `recordCycleStat()` после каждого `run()` (одиночный + loop режим); `AvgEdge` вычисляется из `decisions`, `AvgConfidence` — из FusedForecast когда доступен
- `cmd/dashboard/main.go`: новый subcommand `cycles` → вызов `cmdCycles()` — таблица последних 20 циклов + итоги (avg duration, bets/cycle, edge)

**Файлы:** 3 файла  
**Строк добавлено:** ~150

### TASK-200: /watchdog Telegram + watchdog goroutine
**Что сделано:**
- `internal/metrics/metrics.go`: добавлена функция `LastCycleAt() time.Time` — геттер `state.lastCycleAt`
- `cmd/bot/main.go`: watchdog горутина (ticker = loop_sec); если `time.Since(last) > 2×loop_sec` → `notifier.NotifyError` с сообщением "⚠️ Watchdog: no cycle in Xs"
- `internal/notifier/telegram_commands.go`: добавлен `/watchdog` case в switch + `handleWatchdog(bcfg)` — показывает статус ok/delayed, время последнего цикла, time since, loop interval; добавлен в `/help` список и в заголовочный комментарий файла; добавлен импорт `internal/metrics`
...
## 2026-05-28 09:27 UTC — TASK-201 + TASK-202

### TASK-201: `/countdown` Telegram command
**Что сделано:**
- `internal/notifier/telegram_commands.go`: добавлен `handleCountdown(bcfg BotConfig) string`
  - Фетчит все активные weather-рынки через `markets.GetWeatherMarkets()`
  - Сортирует по `HoursUntilExpiry()` ASC (ближайшие первыми)
  - Urgency badges: 🔴 <2h, 🟡 2-6h, 🟢 6-24h, ⚪ >24h; ‼️ суффикс при <30 мин
  - Показывает до 10 рынков + итог "Closing soon (≤6h): N market(s)"
  - Добавлен case `/countdown` в switch поллера + запись в /help раздел "Markets"
  - Обновлён заголовочный комментарий файла

### TASK-202: Signal entropy tracker
**Что сделано:**
- `internal/collectors/entropy.go` (новый, ~120 строк):
  - `DisagreementReport{City, TempEntropy, RainEntropy, OverallScore, Agreement, Label, SourceCount}`
  - `ForecastDisagreement(ff *FusedForecast) DisagreementReport` — нормализованный stdev температур (÷10°C) и осадков (÷50%) из `PerSourceForecasts`
  - Labels: "consensus" (<0.15), "moderate" (0.15-0.35), "disputed" (>0.35)
  - `LoadDisagreementReports(dataRoot)` — все города из `weather.Cities` через day-0 кэш
  - `FormatDisagreementTable(reports)` — ASCII таблица с emoji badges
- `cmd/dashboard/main.go`: новый subcommand `entropy` → `cmdEntropy(dataRoot)` — таблица с итогом (consensus/moderate/disputed counts)
- `internal/notifier/telegram_commands.go`: `handleForecastQuality` расширен колонкой "Agr%" (source agreement) через `ForecastDisagreement()`

**go build ./...**: ✅ | **go test ./...**: ✅ (все тесты pass)
**Строк кода:** ~220 (новые + изменения)
**Файлы:** 3 файла изменено + 1 новый (entropy.go)

---

## 2026-05-28 09:42 UTC — TASK-203 + TASK-204

### TASK-203: Intra-city signal anti-correlation guard
**Что сделано:**
- `internal/risk/signal_correlation.go` (новый, ~75 строк): `SignalsAntiCorrelated()` + `IntraCitySignalConflict()`
  - 8 анти-коррелированных пар: rain↔sunny, heat↔cold, snow↔heat, fog↔sunny, rain↔dry
  - Возвращает причину для slog
- `internal/risk/signal_correlation_test.go` (новый, ~70 строк): 6 unit-тестов (все pass)
- `cmd/bot/main.go`: проверка `IntraCitySignalConflict` после `CorrelatedCitiesOpen` (TASK-028) в цикле ставок

### TASK-204: Volume-weighted confidence adjustment
**Что сделано:**
- `internal/strategy/strategy.go`: блок volume adjustment перед TASK-185 cross-day
  - VolumeUSDC > 5000 → +0.04 confidence; > 1000 → +0.02; < 50 → -0.03
  - Логируется через `slog.Debug`

**go build ./...**: ✅ | **go test ./...**: ✅ (все тесты pass)
**Строк кода:** ~155 (новые файлы + изменения)
**Файлы:** signal_correlation.go (новый), signal_correlation_test.go (новый), strategy.go, main.go

---

## 2026-05-28 09:50 UTC — TASK-205 + TASK-206

**TASK-205: Source circuit breaker**
- Добавлены константы `TripThreshold = 3`, `TripDuration = 15 * time.Minute`
- Поле `TripUntil time.Time` добавлено в `SourceHealth`
- Новая функция `IsTripped(source, dataRoot string) bool`
- `RecordSourceCall` теперь устанавливает `TripUntil` при `ConsecFails >= 3` и сбрасывает при успехе
- `aggregator.go` `collectSources`: core источники (openmeteo, nasa, noaa, ecmwf, gfs) пропускаются если `IsTripped` → снижает latency цикла при сбоях
- `super_aggregator.go`: аналогичная логика
- Логируется через `slog.Warn` (trip) и `slog.Debug` (skip)

**TASK-206: `/sources` Telegram команда**
- Новая функция `handleSources(bcfg BotConfig) string`
- Таблица: Source | Status | Last OK | Up% | Fails
- Status emoji: ✅ ok, ⚠️ degraded, ❌ down, 🔴 tripped (с минутами до разблокировки)
- Сортировка: tripped → down → degraded → ok
- Итог строка: "X/Y sources healthy"
- Добавлено в dispatch switch и в /help

**go build ./...**: ✅
**Строк кода:** ~120 (изменения в 4 файлах)
**Файлы:** source_health.go, aggregator.go, super_aggregator.go, telegram_commands.go

---

## 2026-05-28 07:58 UTC — TASK-207 + TASK-208 + TASK-209

### TASK-207: `/breakdown` — city×signal матрица P&L
**Что сделано:**
- `internal/notifier/telegram_commands.go`: новая функция `handleBreakdown(bcfg BotConfig) string`
  - Группирует историю ставок по паре (city, signal)
  - Таблица: City | Signal | N | Win% | P&L | ROI% — сортировка по ROI% descending
  - Мин. 2 ставки для отображения, максимум 15 строк
  - Показывает общую статистику: "X bets across Y combinations"
- Добавлен case "/breakdown" в switch поллера
- Добавлен в /help раздел "Analytics"

### TASK-208: Market price momentum detector
**Что сделано:**
- `internal/markets/momentum.go` (новый, ~45 строк):
  - `RecordPrice(conditionID, yesPrice)` — in-memory кэш цен (sync.Map)
  - `PriceDelta(conditionID, currentPrice)` — дельта vs прошлого цикла
  - `IsConfirming(conditionID, currentPrice, ourP, threshold)` — true когда цена движется навстречу ourP на threshold
- `internal/strategy/strategy.go`: в `EvaluateFused` после volume boost
  - `IsConfirming(..., 0.03)` → `ff.Confidence += 0.02` (cap 0.97)
  - `RecordPrice` в конце для следующего цикла
  - Логируется через `slog.Debug("price momentum boost")`

### TASK-209: `/missed` — пропущенные рынки
**Что сделано:**
- `internal/notifier/telegram_commands.go`: новая функция `handleMissed(bcfg BotConfig) string`
  - Загружает today's prediction log (JSONL)
  - Фильтр: только SKIP:* записи
  - Сортировка по max(yesEdge, noEdge) desc — ближайшие к порогу первыми
  - Топ-10 строк: city | signal | reason | edge | confidence
  - Если нет SKIP: "✅ All evaluated markets were bet on today"
- Добавлен case "/missed" в switch поллера
- Добавлен в /help раздел "Analytics"

**go build ./...**: ✅ | **go test ./...**: ✅ (все тесты pass)
**Строк кода:** ~170 (новые функции + изменения)
**Файлы:** momentum.go (новый), telegram_commands.go, strategy.go

---

## 2026-05-28 08:10 UTC — TASK-210 + TASK-211 + TASK-212

Все предыдущие задачи (001–209) выполнены. Придуманы и реализованы 3 новых Telegram команды, дающие быстрый доступ к уже существующей аналитике в пакете calibration.

### TASK-210: `/sharpe` — Sharpe ratio с трендом
**Что сделано:**
- `handleSharpe(bcfg BotConfig) string`:
  - `calibration.RollingSharpe(dataRoot, 30)` — 30-дневное скользящее окно
  - `calibration.RollingSharpe(dataRoot, 7)` — 7-дневное для тренда
  - Тренд: 7d > 30d → "↑ improving", иначе "↓ declining"
  - `calibration.SharpeQuality` → "excellent/good/acceptable/poor"
  - Бенчмарки в выводе (>2.0 excellent, >1.0 good, etc.)

### TASK-211: `/timing` — лучшие/худшие часы
**Что сделано:**
- `handleTiming(bcfg BotConfig) string`:
  - `calibration.LoadHourlyStats` → 24 бакета по UTC часам
  - Топ-3 лучших часа + топ-3 худших (win%, n, множитель Kelly)
  - Текущий UTC час с win% и timing multiplier
  - Guard на минимум 3 ставки (иначе "not enough data")

### TASK-212: `/drawdown` — drawdown от пика
**Что сделано:**
- `handleDrawdown(bcfg BotConfig) string`:
  - `calibration.LoadBankrollHistory` — история снэпшотов
  - Peak = max(all snapshots, current bankroll)
  - `calibration.DrawdownFraction` → процент просадки
  - `calibration.DrawdownMultiplier(dd, 0.20)` → Kelly коэффициент
  - 5-уровневый статус: "at peak", minimal, moderate, significant, severe
  - Recovery: сколько USDC нужно до пика

**go build ./...**: ✅
**Строк кода:** ~175 (новые функции) + dispatch + /help
**Файлы:** `internal/notifier/telegram_commands.go` (единственный файл)

---

## 2026-05-28 10:17 UTC — TASK-213 + TASK-214 + TASK-215

Все предыдущие задачи (001–212) выполнены. Придуманы и реализованы 3 новых Telegram команды для аналитики производительности.

### TASK-213: `/streak` — текущая серия + исторический рекорд
**Что сделано:**
- `handleStreak(bcfg BotConfig) string`:
  - `calibration.ComputeStreak(records)` → текущая серия (N, kind)
  - Сканирование всей истории для нахождения bestWin и worstLoss рекордов
  - `calibration.CurrentStreak` + `calibration.StreakKellyFactor` для Kelly множителя
  - Emoji статус: 🔥 5+ wins, ✅ 1-4 wins, ⚠️ 1-3 losses, 🚨 4+ losses
  - Формат: Current / Best / Worst / Kelly × / Total resolved bets

### TASK-214: `/weekly` — 4-недельная P&L таблица
**Что сделано:**
- `handleWeekly(bcfg BotConfig) string`:
  - `calibration.WeeklyBreakdown(records, 4)` → 4 недели
  - `calibration.BestWeek/WorstWeek` для итоговых строк
  - Таблица: Week | Bets | Win% | P&L; текущая неделя помечена "*"
  - Guard: нужно минимум 2 недели данных

### TASK-215: `/roi` — кумулятивный ROI% от 100 USDC
**Что сделано:**
- `handleROI(bcfg BotConfig) string`:
  - `calibration.DefaultBankroll` (100 USDC) как базовый
  - `calibration.LoadBankroll` → текущий
  - `calibration.WeeklyBreakdown(records, 8)` → 8 недель кумулятивного P&L
  - ASCII sparkline через существующую `asciiSparkline(cumValues)`
  - Показывает: Initial / Current / ROI% / Net P&L / Resolved bets / 8-wk sparkline

**go build ./...**: ✅
**Строк кода:** ~155 (3 новые функции + dispatch cases + /help + комментарии)
**Файлы:** `internal/notifier/telegram_commands.go` (единственный файл)

---

## 2026-05-28 08:40 UTC — TASK-218

### TASK-218: Per-signal calibration error tracker — кривые калибровки по сигналам

**Что сделано:**
- `internal/calibration/calibration_curve.go` (новый):
  - `CalibrationBucket{PredMid, AvgPred, ActualRate, Count}`
  - `BuildCalibrationCurve(records, signal)` — 5 бакетов [0,0.2)…[0.8,1.0], пустой сигнал = все сигналы
  - `CalibrationError(curve)` — взвешенный ECE: mean |avgPred − actualRate|
  - `CalibrationDiagnosis(curve, ece)` — human-readable диагноз: well-calibrated / overconfident / underconfident / etc.
  - `AllSignalCalibrations(records, minBets)` — кривые для всех уникальных сигналов + "all", мин. N ставок
- `internal/calibration/calibration_curve_test.go` (новый):
  - 4 юнит-теста: empty / one-signal / perfect-calibration / systematic-overestimation — все PASS
- `cmd/dashboard/main.go` (обновлён):
  - `cmdCalibrationCurve(dataRoot)` — таблица по каждому сигналу + бакеты + ECE + ASCII-бар (predicted `|` vs actual `*`)
  - `calibrationBar()` — 20-символьный ASCII визуализатор
  - `case "calibration-curve":` в switch + строка в printUsage()
...
## 2026-05-28 08:52 UTC — TASK-220

### TASK-220: Telegram `/compare-signals` — сравнение двух временных периодов по сигналам

**Что сделано:**
- `internal/notifier/telegram_commands.go` (обновлён):
  - `handleCompareSignals(bcfg BotConfig) string` — новая функция
  - Использует `calibration.SignalBreakdownForPeriod(records, 30)` для recent (0-30d) и `60d` для prev
  - prev окно вычисляется как разница: 60d-stats минус 30d-stats = дни 31-60
  - Таблица: Signal | Recent%(N) | Prev%(N) | Δ | Trend emoji
  - Тренд: 📈 если Δ > +5%, 📉 если Δ < -5%, ➡️ иначе
  - N/A если менее 3 resolved ставок в периоде
  - Добавлен case `/compare-signals` в dispatch switch
  - Добавлена строка в `/help` секция Analytics
  - Обновлён header comment файла
...
## 2026-05-28 11:12 UTC — TASK-226

### TASK-226: Source probability spread gate + `/uncertainty` Telegram команда

**Что сделано:**

- `internal/aggregation/spread_gate.go` (новый):
  - `SourceSpread{Min, Max, Spread, Mean, StdDev float64, Sources int}`
  - `ComputeSpread(probs []float64) SourceSpread` — min/max/mean/stddev за O(n)
  - `(s SourceSpread) Exceeds(maxSpread float64) bool` — false при maxSpread<=0 (disabled) или <2 источников
  - `(s SourceSpread) Label() string` → "tight"(<10pp) / "moderate"(10-20pp) / "wide"(20-30pp) / "extreme"(>30pp)

- `internal/aggregation/spread_gate_test.go` (новый):
  - 6 тестов: empty, all-same, two-sources, wide, disabled-gate, label-boundaries
  - Все PASS

- `internal/strategy/strategy.go` (обновлён):
  - `var MaxSourceSpread = 0.35` — пакетная переменная (как KellyFraction)
  - В `EvaluateFused()`: перед ConsensusIndex — `ComputeSpread(probSlice).Exceeds(MaxSourceSpread)` → SKIP:wide_spread
  - `ComputePerSourceProbsRain(perSource)` — экспортированный helper для дождевой вероятности

- `config/config.go` (обновлён):
  - `MaxSourceSpread float64 yaml:"max_source_spread"` (default 0.35)
  - ENV override: `MAX_SOURCE_SPREAD`
  - Дефолт добавлен в `defaults()`

- `config/config.yaml` (обновлён):
  - `max_source_spread: 0.35` с комментарием

- `cmd/bot/main.go` (обновлён):
  - `strategy.MaxSourceSpread = cfg.MaxSourceSpread` при старте

- `internal/notifier/telegram_commands.go` (обновлён):
  - Импорт `aggregation`
  - `case "/uncertainty"` → `handleUncertainty(bcfg)`
  - `handleUncertainty`: таблица city|min%|max%|spread pp|label|⛔ для 9 городов из кэша

**go build ./...**: ✅
**Тесты**: 6/6 PASS (internal/aggregation)
**Строк кода:** ~80 (spread_gate.go) + ~65 (тесты) + ~65 (telegram handler) + ~25 (wiring)
**Файлы:** `internal/aggregation/spread_gate.go`, `internal/aggregation/spread_gate_test.go`, `internal/strategy/strategy.go`, `config/config.go`, `config/config.yaml`, `cmd/bot/main.go`, `internal/notifier/telegram_commands.go`

---

## 2026-05-28 11:25 UTC — TASK-227 + TASK-228

### TASK-227: NWS Active Alerts Telegram `/alerts`
Добавлена команда `/alerts` в Telegram bot. Итерирует 4 US города (new_york, miami, chicago, los_angeles), загружает FusedForecast из кэша, показывает AlertLevel + AlertEvents с emoji-кодированием (🔴=Warning, 🟠=Watch, 🟡=Advisory, 🟢=no alerts). Сортировка по AlertLevel DESC. Добавлена в /help.

### TASK-228: Per-signal 7d win rate trend в `/signals`
Создан `internal/calibration/signal_trend.go` с функциями `SignalTrend7d()` и `FormatTrend()`. Колонка "7dΔ" добавлена в таблицу `/signals`: показывает изменение win rate за последние 7 дней vs предыдущие 7 дней. Требует минимум 3 ставки в каждом окне. Форматирование: "+5% 📈" / "-3% 📉" / "~" / "N/A".

**go build ./...**: ✅
**Строк кода:** ~65 (signal_trend.go) + ~80 (telegram handlers) + ~15 (wiring)
**Файлы:** `internal/calibration/signal_trend.go`, `internal/notifier/telegram_commands.go`, `TASKS.md`

---

## 2026-05-28 09:30 UTC — TASK-229 + TASK-230 + TASK-231

Все задачи из предыдущих приоритетов были выполнены. Созданы и реализованы 3 новых задачи:

### TASK-229: Telegram `/ab-test` — A/B strategy test status
Добавлена команда `/ab-test` в Telegram бот. Показывает сравнительную таблицу A (quarter-Kelly=0.25) vs B (half-Kelly=0.50): кол-во ставок, resolved, win rate, Brier score, ROI%, total P&L. При наличии 50+ resolved ставок выводит победителя с Delta Brier. Данные через `strategy.LoadABStats()` и `strategy.ABWinner()` — вся инфраструктура уже существовала в ab_strategy.go, не хватало только Telegram интерфейса.

### TASK-230: Forecast momentum tracker + `/momentum` команда
- Создан `internal/collectors/momentum_cache.go`:
  - `MomentumPoint{Timestamp, TempC, PrecipMM, PrecipProb}` — один снапшот
  - `SaveMomentum(city, ff, dataRoot)` — сохраняет последние 3 точки в `data/momentum/{city}.json`
  - `MomentumResult{City, TempDir, PrecipDir, TempDelta, PrecipDelta, Points, OldestAt}`
  - `GetMomentum(city, dataRoot) (MomentumResult, bool)` — "rising"/"falling"/"stable" по delta-threshold
  - Пороги: 0.5°C для температуры, 5pp для осадков
- Telegram команда `/momentum`: таблица 9 городов × Temp | Precip с emoji (🔥/❄️/➡️, 🌧/☀️/➡️) и числовым delta

### TASK-231: `/best-city` — топ города по суммарному edge
Добавлена команда `/best-city`. Агрегирует все рынки с positive edge по городам, суммирует totalEdge, показывает топ-3 с медалями 🥇🥈🥉: город, суммарный edge, кол-во рынков. Логика переиспользует код из handleTopEdge без дублирования.

**go build ./...**: ✅
**Строк кода:** ~90 (momentum_cache.go) + ~160 (telegram handlers: ab-test, momentum, best-city) + ~20 (wiring + help)
**Файлы изменены:** `internal/collectors/momentum_cache.go` (новый), `internal/notifier/telegram_commands.go`, `TASKS.md`

---

## 2026-05-28 15:09 UTC — TASK-232 + TASK-233 + TASK-234

Все предыдущие задачи (до TASK-231) были выполнены. Добавлено 3 новые задачи и сразу реализованы.

### TASK-232: Edge-bucket win-rate validator + `/edge-buckets`
Создан `internal/calibration/edge_buckets.go`:
- `EdgeBucket{Label, MinEdge, MaxEdge, Count, Wins, PnL, TotalRisked, BrierSum}` — один тир по размеру edge
- `ComputeEdgeBuckets(records) []EdgeBucket` — 4 фиксированных бакета: <5%, 5-10%, 10-15%, >15%
- `EdgeValidation(buckets) (string, bool)` — проверяет монотонность win rate по edge размеру
- Telegram `/edge-buckets`: таблица бакет|ставок|win%|P&L|ROI% + строка валидации
- 5 unit-тестов: Empty, DistinctBuckets, SkipsUnresolved, AllInOneBucket, NotEnoughData
- Строк: ~115 (edge_buckets.go) + ~55 (тесты) + ~40 (telegram handler)

### TASK-233: P&L по дню недели + `/pnl-weekday`
Создан `internal/calibration/weekday.go`:
- `WeekdayStats{Day, Bets, Wins, PnL, TotalRisked}` + WinPct/ROIPct методы
- `WeekdayBreakdown(records) [7]WeekdayStats` — по UTC дню недели
- `BestWeekday`, `WorstWeekday` с настраиваемым minBets порогом
- Telegram `/pnl-weekday`: таблица Mon-Sun + 🏆 лучший / 📉 худший
- 4 unit-теста: Empty, OneDay, AllDays, BestWorstWeekday
- Строк: ~75 (weekday.go) + ~65 (тесты) + ~45 (telegram handler)

### TASK-234: Price drift tracker + `/drift`
Создан `internal/markets/price_drift.go`:
- `DriftRecord{CondID, Side, EntryPrice, CurrentPrice, DriftPP, CreatedAt, UpdatedAt}` — JSON-персист
- `SaveDriftEntry(condID, side, entryPrice, dataRoot)` — записывает цену входа (один раз)
- `UpdateDrift(condID, currentPrice, dataRoot)` — обновляет текущую цену и drift в pp
- `DriftSummary{Count, Positive, Negative, AvgDrift}` + `LoadDriftSummary(dataRoot)`
- Telegram `/drift`: средний drift, соотношение positive/negative, интерпретация
- 5 unit-тестов: SaveDriftEntry, NoDuplicate, UpdateDrift, Empty, Mixed
- Строк: ~110 (price_drift.go) + ~80 (тесты) + ~45 (telegram handler)

**go build ./...**: ✅
**Все тесты**: 14 новых PASS (5+4+5 + 1 BestWorst)
**Файлы:** `internal/calibration/edge_buckets.go`, `internal/calibration/edge_buckets_test.go`, `internal/calibration/weekday.go`, `internal/calibration/weekday_test.go`, `internal/markets/price_drift.go`, `internal/markets/price_drift_test.go`, `internal/notifier/telegram_commands.go`, `TASKS.md`, `NIGHT_LOG.md`

---

## 2026-05-28 17:29 UTC — TASK-235 + TASK-236 + TASK-237

Все предыдущие задачи выполнены. Добавлено 3 новые задачи и реализованы.

### TASK-235: Probability accuracy distribution + `/prob-dist`
Создан `internal/calibration/prob_dist.go`:
- `ProbBucket` с методами AvgPred/WinPct/ROIPct/CalibrationGap
- `BuildProbDist(records)` — 6 тиров: 0.40-0.50, 0.50-0.55, 0.55-0.60, 0.60-0.70, 0.70-0.80, 0.80-1.00
- `ProbDistECE(buckets)` — Expected Calibration Error (weighted mean |pred-actual|)
- `FormatProbDist(buckets)` — таблица Range|N|Pred|Win%|Gap|ROI% с ✅/🟡/❌ по |gap|<3/7%
- Telegram `/prob-dist`: таблица + ECE label
- 6 unit-тестов PASS

### TASK-236: Bet size distribution + `/size-dist`
Создан `internal/calibration/size_dist.go`:
- `SizeBucket` с методами WinPct/ROIPct/AvgSize
- `ComputeSizeBuckets(records)` — 4 тира: <$1, $1-$2, $2-$5, $5+
- `SizeValidation(buckets)` — проверяет монотонность win rate (Kelly validation)
- `FormatSizeDist(buckets)` — таблица + validation footer
- Telegram `/size-dist`
- 7 unit-тестов PASS (включая исправление бага non-monotone detection)

### TASK-237: Best/worst bets + `/best-bets`
Создан `internal/calibration/best_bets.go`:
- `BetSummary` struct с ROIPct
- `computeBetSummaries(records)` — P&L и ROI для каждой resolved ставки
- `TopBottomBets(records, topN)` — сортировка по ROI%, копирование через make() (фикс shared backing array bug)
- `FormatBestBets(records, topN)` — HTML "🔝 Top / 💀 Worst" с топ-5 каждого
- Telegram `/best-bets`
- 7 unit-тестов PASS

**go build ./...**: ✅
**go test ./...**: ✅ все пакеты
**Строк кода:** ~165 (prob_dist.go) + ~75 (prob_dist_test.go) + ~105 (size_dist.go) + ~85 (size_dist_test.go) + ~130 (best_bets.go) + ~95 (best_bets_test.go) + ~30 (telegram handlers)
**Файлы изменены:** `internal/calibration/prob_dist.go` (новый), `internal/calibration/prob_dist_test.go` (новый), `internal/calibration/size_dist.go` (новый), `internal/calibration/size_dist_test.go` (новый), `internal/calibration/best_bets.go` (новый), `internal/calibration/best_bets_test.go` (новый), `internal/notifier/telegram_commands.go`, `TASKS.md`, `NIGHT_LOG.md`

---

## 2026-05-28 17:32 UTC — TASK-238 + TASK-239

Все предыдущие задачи (до TASK-237) выполнены. Добавлено и реализовано 2 новые задачи.

### TASK-238: Signal burnout detector + `/burnout`
Создан `internal/calibration/burnout.go`:
- `BurnoutConfig{WindowDays=14, FreqMultiplier=2.0, WinRateThreshold=0.45, BurstWindow=48h, BurstLimit=5, MinBets=4}` — настраиваемые пороги
- `BurnoutResult{Signal, BetsWindow, WinRate, Bursting, BurstCount, Overloaded, FreqRatio, AlertMessage}` — результат для одного сигнала
- `AnalyzeBurnout(records, cfg) BurnoutReport` — алгоритм:
  1. Считает resolved ставки каждого сигнала за последние WindowDays дней
  2. Считает all ставки за последние BurstWindow часов (для burst детекции)
  3. Вычисляет globalAvg = total_resolved / num_signals
  4. Overloaded: freqRatio ≥ FreqMultiplier И winRate < WinRateThreshold
  5. Burst: burstCount ≥ BurstLimit
- `FormatBurnout(report)` — ASCII таблица Signal|Bets|WinR%|Burst|Status с секцией алертов
- Telegram `/burnout`: показывает состояние всех сигналов + конкретные предупреждения
- 7 unit-тестов: Empty, NoAlert, Overloaded, Burst, HighFreqGoodRate, UnresolvedIgnoredForWinRate, FormatBurnout_NoAlerts — все PASS
- Строк: ~220 (burnout.go) + ~165 (burnout_test.go)

### TASK-239: `/portfolio` — enriched open positions
Добавлен `handlePortfolio(bcfg)` в `internal/notifier/telegram_commands.go`:
- Загружает историю, вызывает `calibration.FetchUnrealizedPnL()` для всех открытых позиций
- Для каждой позиции: label (City/Sig Side), $size, entry→current price, ΔP&L, timestamp
- Итог: total exposure USDC + total unrealized P&L USDC
- Graceful: если Gamma API недоступен → показывает "N/A" без краша
- Превосходит `/positions` тем, что показывает живые цены и нереализованный P&L
- Строк: ~60 (handler) + switch case + docstring + /help updates

**go build ./...**: ✅
**go test ./...**: ✅ все пакеты
**Файлы изменены:** `internal/calibration/burnout.go` (новый), `internal/calibration/burnout_test.go` (новый), `internal/notifier/telegram_commands.go`, `TASKS.md`, `NIGHT_LOG.md`
